...
func TestInscription(t *testing.T) {
	...
	signingWif, _ := bitcoin.CreateWifString()
	// The key of the wallet to hold the 1sat ordinals
	ordinalWif, _ := bitcoin.CreateWifString()
	...
	// Set ordinal payload
	data := &b.B{
		Data:      b.Data{Bytes: fsBytes},
		MediaType: fireShardMediaType,
		Encoding:  "binary",
	}
	...
	inscription, tx, err := Inscribe(inscribeUtxos, opReturn, paymentPk, changeAddress, ordinalAddress, &signingAddress, &signingKey)
	if err != nil {
